...
'''
Modified version of pDockQ2.py script for operating with PDB/JSON AlphaFold output format
...
def main(model_file, json_file, contact_threshold):
    #args = parser.parse_args()
    #pdbp = PDBParser(QUIET=True)
    cifp = MMCIFParser(QUIET=True)
    #iopdb = PDBIO()

    # print(f"will load {args.pdb[0]}")
    #structure = pdbp.get_structure('', args.pdb[0])
    structure = cifp.get_structure('', model_file)
    ...
    for idx in range(len(chains)):
        ...
        IF_plddt, contact_lst = retrieve_IFplddt(structure, chains[idx], chain2_lst, contact_threshold)
        plddt_lst.append(IF_plddt)
        # print(f"plddt retrieved for chain {chains[idx]}")
        remain_contact_lst.append(contact_lst)
        # print(f"contacts retrieved for chain {chains[idx]}")
    ...
    #data = parse_json(args.json[0])
    data = parse_json(json_file)
    data['predicted_aligned_error'] = np.array(data['pae'])

    avgif_pae = retrieve_IFPAEinter(structure, data['predicted_aligned_error'], remain_contact_lst, contact_threshold)
    ## calculate pmiDockQ

    # print("avgif_pae: ", avgif_pae, "plddt_lst: ", plddt_lst) # one per chain
    ...
    for ch in range(len(chains)):
      rs += f"{chains[ch]} {str(res['pmidockq'].tolist()[ch])}  "
